...
do 
	begin_date=$(date -d "@$month_begin" +%Y-%m-%d)
        echo "month:$begin_date"
	hive --hivevar end_date=${begin_date} --hivevar cycle=1 -f  $project_path/hql_file/aps_order_amount_area_month.sql
	hive --hivevar end_date=${begin_date} --hivevar cycle=1 -f  $project_path/hql_file/aps_order_amount_month.sql

...
do 
	begin_date=$(date -d "@$month_begin" +%Y-%m-%d)
        echo "季：$begin_date"
	hive --hivevar end_date=${begin_date} --hivevar cycle=3 -f  $project_path/hql_file/aps_order_amount_area_month.sql &
	hive --hivevar end_date=${begin_date} --hivevar cycle=3 -f  $project_path/hql_file/aps_order_amount_month.sql

...
do 
	...
	hive --hivevar begin_date=${begin_date} --hivevar cycle=1 -f  $project_path/hql_file/aps_order_amount_month.sql
...
do 
	begin_date=$(date -d "$month_begin" +%Y-%m-%d)
	hive --hivevar begin_date=${begin_date} --hivevar cycle=3 -f  $project_path/hql_file/aps_order_amount_month.sql
...
do 
	begin_date=$(date -d "$month_begin" +%Y-%m-%d)
	hive --hivevar begin_date=${begin_date} --hivevar cycle=12 -f  $project_path/hql_file/aps_order_amount_month.sql
